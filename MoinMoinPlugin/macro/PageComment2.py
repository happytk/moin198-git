...
def execute(macro, args):
    ...
    action = macro.request.form.get('commentaction', '')
    ...
    if action == addcommand:
        ...
        formvals, missingfields = getforminput(macro.request.form, form_fields, required_fields)
...
def getforminput(form, inputfields, requiredfields):
    ...
    for item in inputfields.keys():
        formvals[item] = form.get(item, inputfields[item])
...
def getparams(args):
    ...
    try:
        Params.newerfirst = int(params.get('newerfirst', 1))
    except ValueError:
        Params.newerfirst = 1
    ...
    try:
        Params.notify = int(params.get('notify', 1))
    except ValueError:
        Params.notify = 1
    ...
    try:
        Params.markup = int(params.get('markup', 1))
    except ValueError:
        Params.markup = 1
...
def addcomment(macro, comicon, comauthor, comtext, compasswd, comrev, comautopass, commarkup):
    ...
    parent_acl_string = Page(request, Globs.pagename).parse_processing_instructions()['acl'].getString()
    if not pg.exists():
        action = 'SAVENEW'
        # default acl was Globs.defaultacl
        defaultacl = parent_acl_string 
        warnmessages = '\'\'\'\'\'DO NOT EDIT THIS PAGE!!\'\'\' This page is automatically generated by Page``Comment macro.\'\'\n----'
        # redirect to the client's orignal page, not the comment only page (helps the user when this page shows up as a page-hit)
        redirect = '#redirect %s' % Globs.pagename
...
def patch_acl(parent_acl_string):
    "Patch the ACL line of the datapage with the acl line of the viewing page (current page)" 
    request = Globs.macro.request
    cfg = request.cfg
    _ = request.getText
    ...
    if not lines[0].upper().startswith('#ACL '):
        lines.insert(0,'dummy line')
    lines[0] = parent_acl_string[:-1] # no newline!
    if not lines[1].upper().startswith('#REDIRECT '):
        lines.insert(1,'dummy line')
    lines[1] = "#redirect " + Globs.pagename + '\n'
    newpagetext = '\n'.join(lines)
    while newpagetext[0] == '\n':
        newpagetext = newpagetext[1:]
    newpagetext = pg.normalizeText( newpagetext )
    comment = u'Acl modification at %s' % Globs.curpagename
    pg._write_file(newpagetext, 'SAVE', comment)
    return 1
...
def showcomment():
    ...
    # controleer of de acls'van de data pagina en de view pagina verschillen
    parent_acl_string = Page(request, Globs.pagename).parse_processing_instructions()['acl'].getString()
    datapage_acl_string = Page(request, Globs.datapagename).parse_processing_instructions()['acl'].getString()
    if parent_acl_string != datapage_acl_string:
        # indien dat het geval is: patch de datapagina's acl regel met de acl 's van de view pagina
        patch_acl(parent_acl_string)
...
##    return pg._sendNotification(\
##            comment+'\n\n'+('-'*70)+'\n'+comtext+'\n', \
##            emails, email_lang, [], trivial)
    ...
    mailBody = _("Dear Wiki user,\n\n"
        ...
        "%(pagelink)s\n\n", formatted=False) % {
            ...
            'pagelink': pg.request.getQualifiedURL(pg.url(request,relative=False)),
