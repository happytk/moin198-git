...
from MoinMoin.macro.DiffTest import diff
...
def format_page_edits(macro, lines, bookmark_usecs):
    ...
    difftext = diff(pagename, request, rev1=-len(lines))
    if len(difftext)>0:
            
        ret = '<tr><td colspan="6" style="padding-top:2px;"><b>' + d['icon_html'] + ' '
        ret += d['pagelink_html'] + '</b>'
        ret += difftext
        ret += '</td></tr>'
    
    else:

        ret = request.theme.recentchanges_entry(d)

    return ret
    #return request.theme.recentchanges_entry(d), diff(pagename, request, rev1=-len(lines))

def cmp_lines(first, second):
    return cmp(first[0], second[0])

def macro_RecentChangesDetail(macro, abandoned=False):
